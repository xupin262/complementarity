...
def main(args):
    ...
    # # # 计算加权平均集成方法的权重
    Dice = args.dices  # 各个模型的dice
    logs, weight = [], []
    for dice in Dice:
        logs.append(math.log((1-dice)/dice))
    for i in range(len(Dice)):
        weight.append(logs[i] / sum(logs))
    print("weight: ", weight)
...
# 用法示例如下：
# CUDA_VISIBLE_DEVICES=0 python tools/model_ensemble_wAverage.py \
#   --predict_pt 'work_dirs/pt/mask2former_733_predict.pt'\
#                 'work_dirs/pt/upernet_7191_predict.pt'\
#   --out 'work_dirs/ensemble/ensemble_wAve_mask2former-733_upernet-7191' \
#   --dices    0.733 0.7191
